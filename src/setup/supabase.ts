...
-- Profiles table (uses Clerk user ID as primary key)
...
CREATE TABLE IF NOT EXISTS public.organizations (
  ...
  description TEXT,
  logo_url TEXT,
  owner_id TEXT REFERENCES public.profiles(id) ON DELETE CASCADE NOT NULL, -- Clerk user ID
  created_at TIMESTAMP WITH TIME ZONE DEFAULT NOW(),
  updated_at TIMESTAMP WITH TIME ZONE DEFAULT NOW()
);
...
-- Profiles policies (using Clerk user ID)
...
-- Tasks policies (using Clerk user ID)
...
-- Organizations policies (using Clerk user ID)
...
-- Organization members policies (using Clerk user ID)
    ...
    CASE NEW.event_type
      ...
      WHEN 'user.updated' THEN
        ...
        UPDATE public.profiles SET
          ...
          updated_at = NOW()
        WHERE id = NEW.event_data->>'id';
        
      WHEN 'user.deleted' THEN
        -- Handle user deletion
        DELETE FROM public.profiles WHERE id = NEW.event_data->>'id';
    END CASE;
  END IF;
...
BEGIN
  ...
  RAISE NOTICE '- Clerk user ID integration';
  RAISE NOTICE '=====================================================';
